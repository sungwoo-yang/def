...
/////////////////////////////////////////////////////////////////
Point vertices[] = { Point(1, 1, 1), Point(1, 1, -1), Point(1, -1, 1), Point(1, -1, -1), Point(-1, 1, 1), Point(-1, 1, -1), Point(-1, -1, 1), Point(-1, -1, -1) };
...
private:
    ...
    float current_time, translate_time;
};

Client::Client(SDL_Window* w) : window(w)
{
    int width, height;
    ...
    ndc_to_dev   = translate(Vector(-0.5f, -0.5f, 0)) * scale(0.5f * width, 0.5f * height, 1) * translate(Vector(1, 1, 0));
    ...
    try
    {
        texture[1] = new Texture("texture.bmp");
    }
    catch (exception& e)
    {
        cout << e.what() << endl;
...
void Client::draw(float dt)
{
    ...
    Point cube_center[2];
    cube_center[0] = O + 5 * sin(cube1_rate.x * translate_time) * EX + 5 * sin(cube1_rate.y * translate_time) * EY + 5 * sin(cube1_rate.z * translate_time) * EZ;
    obj_to_world.clear();
    Affine Mo = translate(cube_center[0] - O) * rotate(acos(cube1_axis.z), cross(EZ, cube1_axis)) * rotate(cube1_rate.w * current_time, EZ) * scale(2);
    obj_to_world.push_back(Mo);
    cube_center[1] = O + 5 * sin(cube2_rate.x * translate_time) * EX + 5 * sin(cube2_rate.y * translate_time) * EY + 5 * sin(cube2_rate.z * translate_time) * EZ;
    Mo             = translate(cube_center[1] - O) * rotate(acos(cube2_axis.z), cross(EZ, cube2_axis)) * rotate(cube2_rate.w * current_time, EZ) * scale(2.1f);
    obj_to_world.push_back(Mo);
    ...
    if (intersect)
    {
        for (int n = 0; n < 2; ++n)
        {
            ...
            for (int i = 0; i < 12; ++i)
            {
                ...
                const Point &       P = world_verts[f.index1], &Q = world_verts[f.index2], &R = world_verts[f.index3];
    ...
    for (int n = 0; n < 2; ++n)
    {
        ...
        {
            const TexturedFace& f      = faces[i];
            const Point &       Pworld = world_verts[f.index1], &Qworld = world_verts[f.index2], &Rworld = world_verts[f.index3];
            TexturedCoord       Ptex(Pworld, tex_scale[n] * f.u1, tex_scale[n] * f.v1), Qtex(Qworld, tex_scale[n] * f.u2, tex_scale[n] * f.v2), Rtex(Rworld, tex_scale[n] * f.u3, tex_scale[n] * f.v3);
            clipped_verts.clear();
