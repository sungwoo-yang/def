...
// 2. FillTriangle (PDF 18페이지 원근 보정 보간 반영)
void FillTriangle(Raster& r, const TexturedCoord& P, const TexturedCoord& Q, const TexturedCoord& R) {
    ...
    // [최적화] 루프 밖에서 나눗셈 미리 수행 (No redundant computations)
    ...
    float P_px = (P_dev_x + 1.0f) * 0.5f * width, P_py = (P_dev_y + 1.0f) * 0.5f * height;
    float Q_px = (Q_dev_x + 1.0f) * 0.5f * width, Q_py = (Q_dev_y + 1.0f) * 0.5f * height;
    float R_px = (R_dev_x + 1.0f) * 0.5f * width, R_py = (R_dev_y + 1.0f) * 0.5f * height;

    // [최적화] Bounding Box 계산 (Minimizes comparisons)
    float min_xf = P_px, max_xf = P_px, min_yf = P_py, max_yf = P_py;
    if (Q_px < min_xf) min_xf = Q_px; if (Q_px > max_xf) max_xf = Q_px;
    if (R_px < min_xf) min_xf = R_px; if (R_px > max_xf) max_xf = R_px;
    if (Q_py < min_yf) min_yf = Q_py; if (Q_py > max_yf) max_yf = Q_py;
    if (R_py < min_yf) min_yf = R_py; if (R_py > max_yf) max_yf = R_py;

    int min_x = static_cast<int>(min_xf), max_x = static_cast<int>(max_xf + 1.0f);
    int min_y = static_cast<int>(min_yf), max_y = static_cast<int>(max_yf + 1.0f);
        ...
        for (int px_x = min_x; px_x <= max_x; ++px_x) {
            ...
            if (w0 >= 0.0f && w1 >= 0.0f && w2 >= 0.0f) {
                ...
                if (z_pixel < r.GetZ()) { 
                    ...
                    r.SetColor(static_cast<Raster::byte>(r_s > 255.0f ? 255.0f : (r_s < 0.0f ? 0.0f : r_s)),
                               static_cast<Raster::byte>(g_s > 255.0f ? 255.0f : (g_s < 0.0f ? 0.0f : g_s)),
